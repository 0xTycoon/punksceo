...
async function main() {
    ...
    let events = await CIG.queryFilter('Deposit', 14076792, currentBlock);
    let i = 0;
    console.log(events[8].args.amount);
    while (i < 10) {

        if (bal.hasOwnProperty(events[i].args.user)) {
            bal[events[i].args.user] = bal[events[i].args.user] + events[i].args.amount;
        } else {
            bal[events[i].args.user] = events[i].args.amount;
        }
        i++;
    }

    //events = await CIG.queryFilter('Withdraw', 14076792, currentBlock);
    /*

    {
      '0x0a690B298f84D12414F5c8dB7de1EcE5a4605877': BigNumber { value: "3595451453884134735201" },
      '0x5Da487Ea7278E25288fd4f0f9243e3Fa61bc7443': BigNumber { value: "797346074767093192996" },
      '0xb38071B23F0Fa92cF9e1Bd6FEeC5a5F9821F3CCC': BigNumber { value: "705124142208660066851" },
      '0xc43473fA66237e9AF3B2d886Ee1205b81B14b2C8': BigNumber { value: "1698057713977943682720" },
      '0xF5999070ED2a5A5Fac42bF03C4f8CcFc0090f03F': BigNumber { value: "221706930348661593658" },
      '0xe00FC5e4C8a40022af6135ca4680B4B00fcA217c': BigNumber { value: "61367318928729312072" },
      '0xc3d6E933B2ced203bb1ae344D009849C686161f5': BigNumber { value: "225821079545922037292" },
      undefined: 'undefined11870669548559333730910',
      '0x13aff6B6d6Ad092e8D552Af8FbdE6203770ad0eB': BigNumber { value: "537156567512819642710" },
      '0x01Ec738293d4fA32D99374A4519B450f4DB2bA54': BigNumber { value: "460739213987322653947" }
    }
        ...
        while (i < 100) {
          ...
          //  return;
            ...
            if (bal.hasOwnProperty(events[i].args.user)) {
                bal[events[i].args.user] = bal[events[i].user].sub(events[i].args.amount);
                //console.log("sub");
            } else {
                bal[events[i].args.user] = bal[events[i].args.amount];
            }
